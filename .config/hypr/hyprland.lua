...
function renameWorkspace(wsId)
  local wsIndex = (wsId - 1) % 8 + 1
  local wsConfig = workspaces[wsIndex]
  local wsName = wsConfig.name
  hl.dispatch(hl.dsp.workspace.rename({ workspace = wsId, name = wsName }))
end

function renameWorkspaces()
  for i = 1, 16 do
    local wsIndex = (i - 1) % 8 + 1
    local wsConfig = workspaces[wsIndex]
    local wsName = wsConfig.name
    hl.workspace_rule({ workspace = tostring(i), default_name = wsName })
  end
end
...
smw.max_workspaces({ monitor = 0, max = 8 })
smw.max_workspaces({ monitor = 1, max = 8 })
